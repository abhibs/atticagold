<?php
	...
	if(isset($_POST['sMetal']) && $_POST['sMetal']!=''){
		
		date_default_timezone_set("Asia/Kolkata");
		include("dbConnection.php");
		$branchId = $_POST['sBranch'];
		$date = $_POST['staDate'];
		$metal = $_POST['sMetal'];		
		...
		WHERE t.staDate='$date' AND t.status='Approved' AND t.metal='$metal' AND t.branchId='$branchId'
		...
		if($billCount > 0){
			...
			$pdf->Cell(100, 8, $metal." Send Report", 0, 1, "L", true);
		...
		echo "<script>alert('Please Select A Branch To View Report')</script>";
