		...
		<div class="col-lg-12">
			...
			<div class="col-sm-2">
				<form action="export.php" enctype="multipart/form-data" method="post">
					<input type="hidden" name="branch" value="<?php if(isset($_GET['bran'])){ echo $_GET['bran']; }else { echo "All Branches"; } ?>">
					...
					<button type="submit" name="exportFund" class="btn btn-success btn-block" value="Export Excel" required style="margin-top:23px"><span style="color:#ffcf40" class="fa fa-edit"></span> Export Excel</button>
				</form> 
			</div>
		</div>
		<div class="col-lg-12">
			<div class="hpanel">
				<div class="panel-heading">
					<h3 class="text-success"><i style="color:#990000" class="fa fa-money"></i> Funds Report</h3>
				</div>
							...
							<tbody>
								<?php
									if($search=="" && $search1=="" && $search2==""){
										$sql = mysqli_query($con,"SELECT f.available,f.request,f.type,f.branch,f.customerName,f.customerMobile,f.date,f.status,b.branchName FROM fund f,branch b WHERE f.date='$date' AND f.branch=b.branchId ORDER BY f.id DESC");
									}
									else if($search!="" && $search1!="" && $search2=="All Branches"){
										$sql = mysqli_query($con,"SELECT f.available,f.request,f.type,f.branch,f.customerName,f.customerMobile,f.date,f.status,b.branchName FROM fund f,branch b WHERE f.date BETWEEN '$search1' AND '$search' AND f.branch=b.branchId ORDER BY f.id DESC");
									}
									else if($search!="" && $search1!="" && $search2!="All Branches"){
										$sql = mysqli_query($con,"SELECT f.available,f.request,f.type,f.branch,f.customerName,f.customerMobile,f.date,f.status,b.branchName FROM fund f,branch b WHERE f.date BETWEEN '$search1' AND '$search' AND f.branch='$search2' AND f.branch=b.branchId ORDER BY f.id DESC");
									}
									$i = 1;
