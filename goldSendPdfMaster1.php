...
$billId=$branch1;
//echo $billId;
$sql1=mysqli_query($con,"select staDate from trans where date between '$d1' and '$d2' AND branchId='$billId' group by staDate desc limit 1");
$row111=mysqli_fetch_array($sql1);
$stadate=$row111['staDate'];
$column_slno="";
$id="";
$dat="";
$grossW="";
...
$remarks="";
$purity="";
$query=mysqli_query($con,"SELECT * FROM trans where date between '$d1' and '$d2' AND metal='Gold' AND branchId='$billId' AND sta = 'Checked' and staDate = '$stadate'");
$count=mysqli_num_rows($query);
 
 $sql="select billId, sta, date, SUM(netW) as netW,SUM(grossW) as grossW , SUM(netA) as netA,SUM(grossA) as grossA,COUNT(id) as id from trans where date between '$d1' and '$d2' AND metal='Gold' AND branchId='$billId' AND sta = 'Checked' and staDate = '$stadate'";
...
if($priceId==1 )
	...
	//echo $price;
$sql20="select cash from gold where type='Gold' AND city='$price' order by id DESC";
...
$pdf->MultiCell(195,10,'Gold Send Report:'.$branchName,'0','C');
...
$pdf->SetX(165);
$pdf->Cell(20,7,'Gold Rate',1,0,'C',1);
...
$pdf->SetY($Y_Table9);
$pdf->MultiCell(155,5,'Gold Carrying Person:','0','R');
$pdf->SetY($Y_Table9);
...
$pdf->SetY($Y_Table11);
$pdf->SetX(9);
$pdf->MultiCell(55,5,'DATE: '.$stadate,'0','L');
$pdf->SetY($Y_Table11);
$pdf->MultiCell(155,5,'Place:','0','R');
$pdf->SetY($Y_Table11);
$filename= "GoldSendReport".date('d-m-Y').".pdf";
