							...
							<div class="panel-body">
								<table id="example2" class="table table-striped table-bordered table-hover">
									...
									<tbody>
										<?php
											$i = 1;
											$sqlA = mysqli_query($con,"SELECT expense.*,branch.branchName FROM expense,branch WHERE expense.branchCode=branch.branchId AND expense.status IN ('Verified') AND expense.date='$date'"); 
											while($rowA = mysqli_fetch_assoc($sqlA)){
												echo "<tr>";
												echo "<td>" . $i . "</td>";
												echo "<td>" . $rowA['branchName'] . "</td>";
												echo "<td><a class='btn btn-primary' href='expenseApproveReject.php?appId=".$rowA['id']."' onClick=\"javascript: return confirm('Please confirm To Approve');\"><i style='color:#ffcf40' class='fa fa-check'></i> Approve</a></td>";
												echo "<td><a class='btn btn-danger' href='expenseApproveReject.php?rejId=".$rowA['id']."' onClick=\"javascript: return confirm('Please confirm To Reject');\"><i style='color:#ffcf40' class='fa fa-times'></i>  Reject</a></td>";
												...
												echo "<td><b><a class='btn btn-success' href='editExpense.php?id=".$rowA['id']."'>Edit</a></b></td>";
												echo "</tr>";
												$i++;
											}
										?>
							...
							<div class="panel-body"> 
								<table id="example3" class="table table-striped table-bordered table-hover">
									...
									<tbody>
										<?php
											$i = 1;
											$sqlB = mysqli_query($con,"SELECT expense.*,branch.branchName FROM expense,branch WHERE expense.branchCode=branch.branchId AND expense.status='Approved' AND expense.date='$date'");
							...
							<div class="panel-body"> 
								<table id="example4" class="table table-striped table-bordered table-hover">
									...
									<tbody>
										<?php
											$i = 1;
											$sqlC = mysqli_query($con,"SELECT expense.*,branch.branchName FROM expense,branch WHERE expense.branchCode=branch.branchId AND expense.status='Rejected' AND expense.date='$date'");
