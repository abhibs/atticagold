<?php
	...
	if($_SESSION['usertype'] == "Zonal"){
		$zonalLog = mysqli_fetch_assoc(mysqli_query($con,"SELECT date FROM users WHERE username='".$_SESSION['login_username']."'"));
		if($zonalLog['date'] != $date && $zonalLog['date'] != ''){
			header("location:logout.php");
		}
	}

	if($_SESSION['usertype'] == "ApprovalTeam" || $_SESSION['usertype'] == "Master"){
    	...
    	$customerCount = $customerQuery['blocked'];
	}
?>
<!DOCTYPE html>
<html>
	<head>
		...
		<script>
		    const allowedFileExtensions = /(\.jpg|\.jpeg|\.jiff|\.jfif|\.png|\.pdf|\.xls|\.xlsx|\.excel|.csv|\.doc|\.docx)$/i;
    	</script>
    	<style>
			.dropdown-menu {
			max-height: 500px;
                ...
                <marquee style="font-weight:bold; font-size:20px; color:#900; line-height:30px; letter-spacing:1px; margin-left:250px; margin-right:200px; margin-top:10px;" onmouseover="this.stop();" onmouseout="this.start();">
					...
					<?php
					    if($_SESSION['usertype'] == "Branch"){
					        // echo "Attention BMs!!! Gold & Silver Rate is shown only after Customer Registration and Call from VM to Customer.";
					    }
					?>
				</marquee>
			</div>
			...
			<nav role="navigation">
				...
				<form role="search" class="navbar-form-custom" method="post" action="searchbills.php">
					<div class="form-group">
						<input type="text" placeholder="<?php date_default_timezone_set("Asia/Kolkata"); echo date("l / d-M-Y / h:i A"); ?>" class="form-control" name="search">
					</div>
				</form>
				...
				<div class="navbar-right">
					<ul class="nav navbar-nav no-borders">
					 <!--   <li class="dropdown">-->
						<!--	<a href="xalert.php" title="Alert">-->
						<!--		<b><i style="color:#900" class="fa fa-bell-o"></i></b>-->
						<!--	</a>-->
						<!--</li>-->
						<li class="dropdown">
