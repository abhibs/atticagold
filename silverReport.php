        ...
        <div class="col-lg-12">
            <div class="hpanel">
                <div class="panel-heading">
					<div class="col-sm-12">
						<h3><span style="color:#900" class="fa fa-file-text"></span>  SILVER SEND REPORT </h3>
					</div>
					...
					<form action="" method="GET">
						<div class="col-sm-4">
							<div class="input-group" style="margin-top:23px">
								...
								<input style="font-weight:bold;color:#900" type="text" readonly class="form-control" value="Branch : <?php echo $branchName['branchName']." (".$branchId.")" ?>">
							</div>
						</div>
						<div class="col-sm-3">
						...
						<table id="example2" class="table table-striped table-bordered table-hover">
							<tbody>
								<tr align="center" class="theadRow">
									...
									<td><b><span class="fa fa-money"></span> Silver Rate</b></td>
								...
								<?php
									$i = 1;
									$sql1 = mysqli_query($con,"SELECT billId,grossW,netW,grossA,netA,rate,purity,status,date,time FROM trans WHERE date='$date' AND status='Approved' AND branchId='$branchId' AND metal='Silver' AND sta!='Checked'");
								...
								?>
								<tr class="totalSilver">
									<?php
										$sql2=mysqli_query($con,"SELECT ROUND(SUM(netW),2) AS netW,ROUND(SUM(grossW),2) AS grossW , ROUND(SUM(netA)) AS netA,ROUND(SUM(grossA)) AS grossA,COUNT(id) AS count,AVG(rate) AS rateAvg FROM trans WHERE date='$date' AND branchId='$branchId' AND status='Approved' AND metal='Silver' AND sta!='Checked'");
						...
						<form method="POST" action="newCheck.php">
							<table id="example2" class="table table-striped table-bordered table-hover">
								<tbody>
									...
									<?php
										$i = 1;
										$sql1 = mysqli_query($con,"SELECT id,billId,grossW,netW,grossA,netA,rate,purity,status,date,time FROM trans WHERE date BETWEEN '$search1' AND '$search' AND branchId='$branchId' AND metal='Silver' AND status = 'Approved' AND sta!='Checked'");
										while($row1 = mysqli_fetch_assoc($sql1)){
											echo "<tr>";
											echo "<td style='width:50px'>" . $i . "</td>";
											echo "<td>" . $row1['billId'] . "</td>";
											...
											echo "<td>" . round($row1['purity'],2) . "</td>";
											echo "<td>" . $row1['status'] . "</td>";
											echo "<td><input type='checkbox' id='muls[]' name='muls[]' value='".$row1['id']."'/></td>";
											echo "<td></td>";
											echo "</tr>";
											$i++;
										}
									...
									<tr class="totalSilver">
										<?php
											$sql2=mysqli_query($con,"SELECT ROUND(SUM(netW),2) AS netW,ROUND(SUM(grossW),2) AS grossW , ROUND(SUM(netA)) AS netA,ROUND(SUM(grossA)) AS grossA,COUNT(id) AS count,AVG(rate) AS rateAvg FROM trans WHERE branchId='$branchId' AND date BETWEEN '$search1' AND '$search' AND status='Approved' AND metal='Silver' AND sta!='Checked'");
											$row2 = mysqli_fetch_assoc($sql2);
											if($row2['netW'] != 0){
											    $pur = ($row2['grossA']/$row2['netW'])/$row2['rateAvg']*100;
											}
									...
									<?php echo "<tr><td colspan='12'><input type='submit' class='btn btn-success' id='delsilver' name='delsilver' value='Print Report' > <a href='silverSendPdf.php' target='_blank' class='btn btn-primary'><i style='color:#fa0' class='fa fa-print'></i> Re Print Silver Send Report</a></td></tr>"; ?>
								</tbody>
							</table>
						</form>
					<?php } ?>
