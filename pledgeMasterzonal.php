...
if (isset($_GET['start_date']) && isset($_GET['end_date']) || $type == "Zonal"  ) {
   
    $start_date = isset($_GET['start_date']) ? $_GET['start_date'] : $currentDate;
    $end_date = isset($_GET['end_date']) ? $_GET['end_date'] : $currentDate;

    $searchQuery = mysqli_query($con, "SELECT * FROM pledge_bill r WHERE date BETWEEN '$start_date' AND '$end_date' AND status!='Rejected' $state ORDER BY id DESC");
    ...
    $searchQuery = mysqli_query($con, "SELECT * FROM pledge_bill WHERE date = '$currentDate' $state status!='Rejected' ORDER BY id DESC");
											...
											<?php
												...
												// 		$rowColor = (strpos($invoiceNumber, 'HO') === 0) ? 'background-color: #ffeeba;' : '';
                                                     ...
                                                     if ($daysDifference < 30) {
														...
														// release Date
															$releaseQuery = mysqli_query($con, "SELECT date FROM pledge_fund WHERE billId='$billId' AND status='Released'");
														...
														// Released gold Employee data
															...
															$releaseEmp = mysqli_query($con, "SELECT empId,empName FROM pledge_fund WHERE billId='$billId' AND status='Released'");
...
<script>
	...
	function searchByDateRange() {
		...
		window.location.href = "pledgeMasterzonal.php?start_date=" + start_date + "&end_date=" + end_date;
    }
