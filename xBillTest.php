						...
						<div role="tabpanel" class="tab-pane fade in active" id="trans-panel">
							...
							<table id="total-trans-datatable" class="table table-bordered">
								...
								<tbody>
									<?php
										$i = 1;
										$query ="SELECT B.state,B.branchName,T.id,T.billId,T.name,T.customerId,T.phone,T.billCount,T.releases,T.grossW,T.netW,T.grossA,T.netA,T.amountPaid,T.rate,T.time,T.date,T.branchId,T.type,T.comm,T.margin,T.releaseID FROM trans T,branch B WHERE T.date='$date' AND T.status IN ('Pending','Approved') AND T.branchId=B.branchId ORDER BY T.id DESC";
										...
										while($row = mysqli_fetch_assoc($sql)){
											if($row['billCount'] > 1 ){
												...
												if($row['billCount']>1 && $row['billCount']<5){
													$class="row-warning";
												}												
												if($row['billCount']>4){
													$class="row-danger";
												...
												if($row['billCount'] > 1){
													echo "<td class='text-center'><a style='font-size: 12px;' class='btn btn-success btn-md' target='_blank' href='existing.php?phone=".$row['phone']."'>" .$row['billCount']. "</a></td>";
												}
												else{
